...
import { User, Briefcase, BarChart2, LogOut, PlusCircle, CalendarCheck2, ChevronDown, Settings as SettingsIcon, Landmark, Building2, MapPin } from 'lucide-react'
import { useState } from 'react'
import { Header,DasboardSelector } from './component';
...
const Dashboard = () => {
  ...
  const [selectedSection, setSelectedSection] = useState<'dashboard' | 'jobs' | 'settings'>('dashboard')
  ...
  const [selectedSettings, setSelectedSettings] = useState<'bank' | 'city' | 'state' | 'company' | 'skills' | 'shift' | 'department' | null>(null)
  ...
  const handleUpdateBank = (e: React.FormEvent) => {
    ...
    if (editBankId !== null && editBankName.trim()) {
      setBanks(banks.map(b => b.id === editBankId ? { ...b, name: editBankName.trim() } : b))
  ...
  const handleCreateCity = (e: React.FormEvent) => {
    e.preventDefault()
    if (newCityName.trim()) {
      const newId = cities.length ? Math.max(...cities.map(c => c.id)) + 1 : 1
  ...
  const handleUpdateState = (e: React.FormEvent) => {
    ...
    if (editStateId !== null && editStateName.trim()) {
      setStates(states.map(s => s.id === editStateId ? { ...s, name: editStateName.trim() } : s))
  ...
  const handleUpdateCompany = (e: React.FormEvent) => {
    ...
    if (editCompanyId !== null && editCompanyName.trim()) {
      setCompanies(companies.map(c => c.id === editCompanyId ? { ...c, name: editCompanyName.trim() } : c))
  ...
  const handleUpdateSkill = (e: React.FormEvent) => {
    ...
    if (editSkillId !== null && editSkillName.trim()) {
      setSkills(skills.map(s => s.id === editSkillId ? { ...s, name: editSkillName.trim() } : s))
  ...
  const handleUpdateShift = (e: React.FormEvent) => {
    ...
    if (editShiftId !== null && editShiftName.trim()) {
      setShifts(shifts.map(s => s.id === editShiftId ? { ...s, name: editShiftName.trim() } : s))
  ...
  const handleUpdateDepartment = (e: React.FormEvent) => {
    ...
    if (editDepartmentId !== null && editDepartmentName.trim()) {
      setDepartments(departments.map(d => d.id === editDepartmentId ? { ...d, name: editDepartmentName.trim() } : d))
          ...
          <ul className="flex flex-col gap-1">
            <li>
              ...
              >
                ...
                <ChevronDown className={`w-4 h-4 ml-2 transition-transform ${selectedSection === 'settings' ? 'rotate-180' : ''}`} />
              </button>
              {selectedSection === 'settings' && (
                <ul id="sidebar-settings-sublist" className="ml-8 mt-1 flex flex-col gap-1 border-l border-blue-100 pl-4">
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleBankClick}>
                      <Landmark className="w-4 h-4" /> Bank
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleCityClick}>
                      <Building2 className="w-4 h-4" /> City
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleStateClick}>
                      <MapPin className="w-4 h-4" /> State
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleCompanyClick}>
                      <Briefcase className="w-4 h-4" /> Company
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleSkillsClick}>
                      <BarChart2 className="w-4 h-4" /> Skills
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleShiftClick}>
                      <CalendarCheck2 className="w-4 h-4" /> Shift
                    </button>
                  </li>
                  <li>
                    <button className="flex items-center gap-2 px-2 py-1 text-slate-600 hover:text-blue-700 hover:bg-blue-50 rounded transition w-full text-sm" onClick={handleDepartmentClick}>
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-10 bg-slate-100">
          {selectedSection === 'dashboard' && (
            <DasboardSelector data={data} activities={activities} tasks={tasks} />
          )}
          ...
          {selectedSection === 'settings' && selectedSettings === 'bank' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={handleAddBank} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddBankModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddBankModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New Bank</h3>
                    <form onSubmit={handleCreateBank} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddBankModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditBankModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditBankModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    ...
                    <form onSubmit={handleUpdateBank} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditBankModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
                  ...
                  <ul className="divide-y divide-slate-100">
                    {banks.map(bank => (
                      <li key={bank.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{bank.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditBank(bank.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
                          <button onClick={() => handleDeleteBank(bank.id)} className="text-red-500 hover:text-red-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M6 18L18 6M6 6l12 12' /></svg></button>
                        </div>
                      </li>
                    ))}
                  </ul>
          ...
          {selectedSection === 'settings' && selectedSettings === 'city' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={handleAddCity} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddCityModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddCityModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New City</h3>
                    <form onSubmit={handleCreateCity} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddCityModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditCityModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditCityModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    ...
                    <form onSubmit={handleUpdateCity} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditCityModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow p-6 border border-slate-100">
                {cities.length === 0 ? (
                  ...
                  <ul className="divide-y divide-slate-100">
                    {cities.map(city => (
                      <li key={city.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{city.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditCity(city.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
                          <button onClick={() => handleDeleteCity(city.id)} className="text-red-500 hover:text-red-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M6 18L18 6M6 6l12 12' /></svg></button>
                        </div>
                      </li>
                    ))}
                  </ul>
          ...
          {selectedSection === 'settings' && selectedSettings === 'state' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={handleAddState} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddStateModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddStateModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New State</h3>
                    <form onSubmit={handleCreateState} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddStateModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditStateModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditStateModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Edit State</h3>
                    <form onSubmit={handleUpdateState} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditStateModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
              ...
              <div className="bg-white rounded-xl shadow p-6 border border-slate-100">
                {states.length === 0 ? (
                  ...
                  <ul className="divide-y divide-slate-100">
                    {states.map(state => (
                      <li key={state.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{state.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditState(state.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
                          <button onClick={() => handleDeleteState(state.id)} className="text-red-500 hover:text-red-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M6 18L18 6M6 6l12 12' /></svg></button>
                        </div>
                      </li>
                    ))}
                  </ul>
          ...
          {selectedSection === 'settings' && selectedSettings === 'company' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={handleAddCompany} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddCompanyModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddCompanyModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New Company</h3>
                    <form onSubmit={handleCreateCompany} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddCompanyModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditCompanyModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditCompanyModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    ...
                    <form onSubmit={handleUpdateCompany} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditCompanyModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
                  ...
                  <ul className="divide-y divide-slate-100">
                    {companies.map(company => (
                      <li key={company.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{company.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditCompany(company.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
          ...
          {selectedSection === 'settings' && selectedSettings === 'skills' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={handleAddSkill} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddSkillModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddSkillModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New Skill</h3>
                    <form onSubmit={handleCreateSkill} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddSkillModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditSkillModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditSkillModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    ...
                    <form onSubmit={handleUpdateSkill} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditSkillModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
                  ...
                  <ul className="divide-y divide-slate-100">
                    {skills.map(skill => (
                      <li key={skill.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{skill.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditSkill(skill.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
          ...
          {selectedSection === 'settings' && selectedSettings === 'shift' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={handleAddShift} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddShiftModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddShiftModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New Shift</h3>
                    <form onSubmit={handleCreateShift} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddShiftModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditShiftModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditShiftModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    ...
                    <form onSubmit={handleUpdateShift} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditShiftModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
                  ...
                  <ul className="divide-y divide-slate-100">
                    {shifts.map(shift => (
                      <li key={shift.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{shift.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditShift(shift.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
          ...
          {selectedSection === 'settings' && selectedSettings === 'department' && (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-slate-800">Department List</h2>
                <button onClick={handleAddDepartment} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold shadow transition">
              ...
              {showAddDepartmentModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseAddDepartmentModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Create New Department</h3>
                    <form onSubmit={handleCreateDepartment} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseAddDepartmentModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Create</button>
                      </div>
                    </form>
                  </div>
              ...
              {showEditDepartmentModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
                  <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative">
                    <button onClick={handleCloseEditDepartmentModal} className="absolute top-3 right-3 text-slate-400 hover:text-slate-700 text-xl font-bold">&times;</button>
                    <h3 className="text-xl font-bold mb-4 text-slate-800">Edit Department</h3>
                    <form onSubmit={handleUpdateDepartment} className="flex flex-col gap-4">
                      ...
                      />
                      <div className="flex gap-2 justify-end">
                        <button type="button" onClick={handleCloseEditDepartmentModal} className="px-4 py-2 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold">Cancel</button>
                        <button type="submit" className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold">Update</button>
                      </div>
                    </form>
                  </div>
                  ...
                  <ul className="divide-y divide-slate-100">
                    {departments.map(department => (
                      <li key={department.id} className="flex items-center justify-between py-3">
                        <span className="flex-1 truncate text-slate-800 font-medium">{department.name}</span>
                        <div className="flex gap-2">
                          <button onClick={() => handleEditDepartment(department.id)} className="text-blue-500 hover:text-blue-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M15.232 5.232l3.536 3.536M9 13h3l8-8a2.828 2.828 0 00-4-4l-8 8v3z' /></svg></button>
                          <button onClick={() => handleDeleteDepartment(department.id)} className="text-red-500 hover:text-red-700 px-2 py-1 rounded transition"><svg xmlns='http://www.w3.org/2000/svg' className='h-4 w-4' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M6 18L18 6M6 6l12 12' /></svg></button>
                        </div>
                      </li>
                    ))}
                  </ul>
